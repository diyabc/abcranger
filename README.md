...
To get $W$ so that $T=XW$ we compute : $$\mathbf{W}=\mathbf{W}^{*}\left(\widetilde{\mathbf{P}} \mathbf{W}^{*}\right)^{-1}$$ where $\widetilde{\mathbf{P}}_{K \times p}=\mathbf{t}\left[p_{1}, \ldots, p_{K}\right]$ where $\mathbf{W}^{*}_{p \times K} = [w_1, \ldots, w_K]$
